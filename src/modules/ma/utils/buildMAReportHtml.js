...
function renderGovernanceChecklist(meta = {}) {
  const items = [
    {
    ...
    {
      label: 'Distribution',
      value: 'Authorized recipients only'
    },
    {
      label: 'Human review',
      value: 'Required before reliance'
    },
    {
      label: 'Generated date',
      value: meta.generatedDateLabel
    }
  ];
      ...
      ${safeItems
        .map((item, index) => {
          const title =
            typeof item === 'string'
              ? `Item ${index + 1}`
              : safeText(item.title || item.label || `Item ${index + 1}`);

          const body =
            typeof item === 'string'
              ? item
              : safeText(item.content || item.description || item.text || item.value, 'Pending further detail.');
...
function renderDecisionSources(report = {}) {
  const sources = safeArray(report.decisionSourcePack || report.sections?.decisionSourcePack);

  return renderTable(
    ['Control', 'Decision Input', 'Documents', 'Status'],
    sources.map((item) => [
      safeText(item.sourceId, 'ma.source.pending'),
      safeText(item.label || item.evidence, 'Decision source'),
      safeArray(item.documents)
        .map((document) => safeText(document.title, 'Evidence document'))
        .join(', ') || safeText(safeArray(item.requiredDocuments).join(', '), 'Required'),
      safeText(item.status, 'Document validation required')
    ]),
    'No decision source pack available.'
  );
}
...
<head>
  ...
  <meta name="classification" content="Confidential M&A decision-support report" />
  <meta
    name="description"
    content="${escapeHtml(meta.companyName)} confidential M&A valuation, equity bridge, buyer matching, risk review and evidence control report generated by CEO's OS."
  />
  <title>${escapeHtml(meta.companyName)} | Confidential M&A Executive Report</title>
  <style>
    :root {
      --ink: #e5eefc;
      --ink-dark: #0f172a;
  ...
  <main class="report-shell">
    <section class="cover">
      ...
      <div class="cover-main">
        ...
        <h1>
          ...
          <span>Confidential M&A Executive Report</span>
        </h1>
    ...
    <section class="content">
      ...
      <div class="disclaimer-banner">
        <strong>Confidentiality, Governance & Review Notice</strong>
        ${escapeHtml(
          report.disclaimer ||
        ...
        Distribution must remain controlled by the organization. External circulation should require NDA coverage, authorized recipients, version control and professional review.
      </div>

      ${renderGovernanceChecklist(meta)}
      ...
      ${renderSection(
        ...
        'Documentary controls supporting valuation conclusions and risk signals.',
        renderDecisionSources(report),
