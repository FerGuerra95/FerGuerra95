    ...
    <div className="page">
      ...
      <div className="strategy-page">
        <section className="strategy-hero ceos-ws-hero">
          ...
          <p className="strategy-copy">Objectives, initiatives, scenarios, competitive notes, strategic risks and strategy board review draft reporting. Decision support only — human review required.</p>
        </section>
...
export const StrategyReportsPage = () => <EntityPage badge="Reports" title="Strategy reports." copy="Strategy Board Memo, Strategic Scenario Pack, Strategic Execution Report and Capital Allocation Memo — draft metadata only, not exported documents." load={strategyApi.listReports} create={strategyApi.createReport} defaults={{ title: 'Strategy Board Memo', reportType: 'strategy_board_memo' }} fields={['title', 'reportType']} permission={PERMISSIONS.EXPORT_STRATEGY} render={(items) => <StrategyTable title="Strategy reports" items={items} columns={[{ key: 'title', label: 'Report' }, { key: 'reportType', label: 'Type' }, { key: 'status', label: 'Status', render: (item) => <StrategyStatusBadge status={item.status} /> }, { key: 'createdAt', label: 'Prepared metadata' }]} />} />;
