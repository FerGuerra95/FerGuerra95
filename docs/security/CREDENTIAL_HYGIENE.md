...
| **Reason** | A production or shared test password was previously exposed outside secret stores (e.g. chat). Treat as **compromised** until rotated. |
| **Resolution criteria** | Operator rotates password via secure admin/panel; smoke uses `CEOS_E2E_*` env vars only; no password in repo/docs/chat |
