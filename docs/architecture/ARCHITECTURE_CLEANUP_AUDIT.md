...
## C.14.11 - Duplicate Helpers Audit/Fix Results

**Status:** COMPLETED / AUDIT ONLY / NO SAFE CONSOLIDATION.

Batch B audited helper duplication across frontend and backend. No helper consolidation was applied because the duplicate-looking helpers carry module-specific output, null semantics, export formatting, report copy, or scoring boundaries. Consolidating them in this phase would create a higher risk than leaving them documented.

| Pattern | Locations | Classification | Action | Risk | Follow-up |
|---|---|---|---|---|---|
| `formatCurrency` report/export helpers | `src/shared/utils/formatCurrency.js`, `src/modules/funding/services/fundingExportApi.js`, `src/modules/pmi/services/pmiExportApi.js`, `src/modules/ma/utils/buildMAReportHtml.js`, `src/modules/ma/utils/formatMAReportData.js`, `src/modules/ma/pages/ValuationPage.jsx`, `src/modules/ma/components/EquityHeroCard.jsx`, `src/modules/compliance/pages/RiskMapPage.jsx` | DEFER_SEMANTICALLY_DISTINCT | No code change. | Report/export helpers differ in currency symbol placement, `Intl` style, fallback behavior, and module copy expectations. | Create a formatter contract test before any consolidation. |
| `formatPercent` / percent display helpers | `src/shared/utils/formatPercent.js`, `src/shared/utils/formatMetric.js`, `src/modules/funding/services/fundingExportApi.js`, `src/modules/funding/pages/FundingDashboardPage.jsx`, `src/modules/pmi/engine/usePMIEngine.js`, `src/modules/ma/utils/formatMAReportData.js`, `src/modules/ma/engine/reportBuilder.js` | DEFER_SEMANTICALLY_DISTINCT | No code change. | Some helpers map `0-1` ratios to percentages, some preserve raw percent points, and PMI has explicit zero-denominator `N/A` semantics. | Audit per-module percent semantics before shared imports. |
| `parseNumber` / `toNumber` / `safeNumber` | `src/shared/utils/parseNumber.js`, `src/shared/utils/formatMetric.js`, Funding pages/services, PMI services/engine, Compliance engine/services, M&A report builders | DEFER_SEMANTICALLY_DISTINCT | No code change. | Shared `parseNumber` supports locale separators; local `toNumber` helpers intentionally coerce invalid values to `0` for display/export. | Consolidate only after edge-case fixtures for empty string, null, comma decimals, thousands separators, and invalid text. |
| `clamp` / `clampScore` | `src/shared/utils/validators.js`, Compliance/M&A/Funding engines, CEO components, backend executive/reporting/governance/risk/pmi/bridge/heritage services | DEFER_HIGH_RISK | No code change. | Score clamps touch formulas, operational scoring, Golden-adjacent helpers, backend readiness, and UI geometry. | Do not consolidate in cleanup; handle only in explicit scoring/formula phases. |
| `insufficient_data`, `N/A`, null display and fallback labels | CEO truthfulness utilities, backend executive/readiness/reporting services, Reporting/Strategy/Risk enterprise components, Compliance/Funding/M&A/PMI exports | DEFER_SEMANTICALLY_DISTINCT | No code change. | Visual `N/A` is not equivalent to backend null, DSS insufficient data, API fallback, or human-review status. | Define a truthfulness label contract before shared fallback helpers. |
| `humanReviewRequired` / Board Review Draft copy | CEO truthfulness utilities, Risk report truthfulness, backend strategy/executive/pmi/reporting services, report/export builders | DEFER_HIGH_RISK | No code change. | Copy and booleans affect product truthfulness and sales/legal positioning. | Preserve as explicit DSS/human-review wording until a truthfulness-copy phase. |
| Backend `clampScore` mirrors | Backend services for bridge, ecosystem, executive, governance, heritage, pmi, reporting, risk, strategy, compliance | DEFER_HIGH_RISK | No code change. | Backend service helpers participate in API semantics and readiness/status calculations. | Consider backend utility extraction only with per-service contract tests. |
| Shared utility inventory | `src/shared/utils/formatCurrency.js`, `formatPercent.js`, `formatMetric.js`, `parseNumber.js`, `validators.js`; no active backend `utils` helper files found in the targeted listing. | NO_ACTION | No code change. | Existing shared helpers are useful, but not universally equivalent to module-local helpers. | Keep shared helpers as opt-in until equivalence is proven. |

### C.14.11 Decision

No safe 1-3 helper consolidations met the equivalence bar. This phase intentionally made no runtime helper, formula, Golden Dataset, Formula Registry, source-of-truth, scoring, API contract, auth, router, storage, package/config, or UI visual changes. Future consolidation should start with focused contract tests for the exact formatter or parser behavior being replaced.
